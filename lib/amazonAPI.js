...
ApiHelper.defaultResponseGroupSmall = 'Small,Offers';
            ...
            if (error !== false) {
                ...
                if (items != null && items instanceof Array ) {
                    items.forEach(function (entry) {
                        var asin = entry.ASIN,
                            //How cold this possibly be undefined....
                            title = entry.ItemAttributes ? entry.ItemAttributes.Title : '',
                            product = new ApiHelper.Product(asin, title);
                            if(entry.OfferSummary){
                                if(entry.OfferSummary.LowestNewPrice){
                                    product.Price = entry.OfferSummary.LowestNewPrice.FormattedPrice;
                                }
                            }
    ...
    if (productInfo.ItemAttributes) {
        ...
        if (productInfo.ItemAttributes.Creator) {
            ...
            if (productInfo.ItemAttributes.Creator instanceof Array) {
                ...
                product.Creator.push({
                    role: productInfo.ItemAttributes.Creator.$.Role,
                    name: productInfo.ItemAttributes.Creator._
                });
            }
        ...
        if (productInfo.ItemAttributes.Author) {
            ...
            product.Creator.push({
                role: 'Autor',
                name: productInfo.ItemAttributes.Author
            });
        }
...
/*
 * All Package Categories from Amazon
 * For more info see:http://docs.aws.amazon.com/AWSECommerceService/latest/DG/DESearchIndexParamForItemsearch.html
 */
module.exports.AmazonSearchIndices = ["All",
    "Apparel",
    "Automotive",
    "Baby",
    "Blended",
