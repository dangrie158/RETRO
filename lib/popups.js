...
var ConfirmPopup = function (options) {
    ...
    //TODO: No matter how we check for keypresses, we never get them
    this.on('key left', function () {
        self.focusPrevious();
    });
    this.on('key right', function () {
        self.focusNext();
...
var InputPopup = function (options) {
    ...
    //hopfully...
    this.submitButton.once('press', function () {
        self.submit();
    });

    //Give the inpu the focus but dont focus it directly
    //otherwise the form tries to focus it the next time
    this.once('focus', function () {
        self.focusNext();
    });

    //On enter we want to focus out button
    //TODO: Or do we want to submit the form?
    this.input.onceKey('enter', function () {
        self.focusNext();
    });
}
